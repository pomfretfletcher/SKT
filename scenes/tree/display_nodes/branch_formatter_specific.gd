...
func _ready() -> void:
	if not tree.draw_tree.is_connected(format_visuals_of_branch):
		tree.draw_tree.connect(format_visuals_of_branch)


func format_visuals_of_branch():
	for branch in tree.branches_parent.get_branches():
