...
func _ready() -> void:
	...
	if not tree.request_load_data.is_connected(load_data):
		tree.request_load_data.connect(load_data)
...
func save_data() -> void:
	...
	MessageLogger.log_process("Saving Node Data")
	...
	MessageLogger.log_process("Saving System Data")
	var system_saved_data: Dictionary
	system_saved_data.set(
		"progression_tier_manager",
		{
			"uses_tiers": tree.progression_tier_manager.use_progression_tiers,
			"current_tier": tree.progression_tier_manager.current_tier,
		},
	)
	system_saved_data.set(
		"upgrade_cost_manager",
		{
			"upgrade_type": tree.upgrade_cost_manager.upgrade_type,
			"current_sp": tree.upgrade_cost_manager.current_sp,
		},
	)
	system_saved_data.set(
		"grid_node_locker",
		{
			"uses_lock": tree.grid_node_locker.lock_nodes_to_grid,
			"cell_size": tree.grid_node_locker.grid_cell_size,
		},
	)
...
func load_data():
	...
	# setup yet
	tree.request_toggle_tree_updating.emit(false)
	tree.request_toggle_tree_drawing.emit(false)

	MessageLogger.log_process("Clearing Current Controls")
	# Delete all current skill tree controls. Will be remade within loading process
	for child in tree.nodes_parent.get_nodes():
		if child is SkillNode:
			child.free()
	for child in tree.branches_parent.get_branches():
...
func recreate_nodes(node_saved_data: Dictionary, node_dict: Dictionary[String, SkillNode]):
	for node_key in node_saved_data:
		...
		var node: SkillNode = tree.spawner.create_node()
		...
		node.skill_data.setup_data(node)
	...
	for node_key in node_saved_data:
		...
		for p_branch in prev_branches:
			...
			if branch_name not in branch_dict:
				branch = tree.spawner.create_branch()
				...
				branch.unlock_condition.setup_data(branch)
...
func recreate_follow_points_for_branch(branch: SkillBranch, save_data_entry: Dictionary):
	...
	for follow_point_name in follow_point_positions:
		...
		var follow_point: BranchFollowPoint = tree.spawner.create_follow_point(branch)
...
func load_system_data(system_saved_data: Dictionary):
	...
	tree.progression_tier_manager.use_progression_tiers = ptm_data["uses_tiers"]
	tree.progression_tier_manager.current_tier = int(ptm_data["current_tier"])
	...
	tree.upgrade_cost_manager.upgrade_type = SKT_UpgradeCostManager.UpgradeType[SKT_UpgradeCostManager.UpgradeType.find_key(upgrade_type)]
	tree.upgrade_cost_manager.current_sp = int(ucm_data["current_sp"])

	# Grid Node Locker
	var gnl_data: Dictionary = system_saved_data["grid_node_locker"]
	tree.grid_node_locker.lock_nodes_to_grid = gnl_data["uses_lock"]
	tree.grid_node_locker.grid_cell_size = gnl_data["cell_size"]
