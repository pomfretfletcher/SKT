...
@export var current_tier: int:
	set(v):
		...
		for node in tree.nodes_parent.get_nodes():
			if node.is_unlocked() and node.progression_tier > v:
				MessageLogger.log_issue("Cannot change current tier as there are unlocked nodes with a progression tier that would be invalid at the new tier.")
				return
		current_tier = v
...
func _ready() -> void:
	if not tree.update_tree.is_connected(display_tiers):
		tree.update_tree.connect(display_tiers)
		
	tree.request_prepare_for_load.connect(
		func():
			_setup = false
	)
	tree.data_loaded.connect(
		func():
			_setup = true
	)
	display_tiers()
...
func display_tiers():
	...
	if not use_progression_tiers or tree.progression_tier_manager == null:
		return
	for node in tree.nodes_parent.get_nodes():
