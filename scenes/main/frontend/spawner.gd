...
func create_node() -> SkillNode:
	...
	SKT.nodes_parent.add_child(node)
	node.owner = get_tree().edited_scene_root

	# Formats skill node name
	node.name = "SkillNode" + str(SKT.nodes_parent.node_count)
...
func create_branch() -> SkillBranch:
	...
	SKT.branches_parent.add_child(branch)
	branch.owner = get_tree().edited_scene_root

	# Formats branch name
	branch.name = "SkillBranch" + str(SKT.branches_parent.branch_count)
