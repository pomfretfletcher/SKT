...
func _ready() -> void:
	...
	SkillTreeRequests.request_prepare_for_save.connect(
		func():
			reset_node_positions()
			if lock_nodes_to_grid:
				lock_nodes_to_grid = false
				await SkillTreeEvents.data_saved
				lock_nodes_to_grid = true
	)

	TreeInteractionSignals.node_moved.connect(
		func(node: SkillNode):
			if node in non_locked_node_positions.keys():
				non_locked_node_positions.set(node, node.position)
	)
	TreeInteractionSignals.followpoint_moved.connect(
