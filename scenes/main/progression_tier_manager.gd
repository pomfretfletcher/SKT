...
@export var current_tier: int:
	set(v):
		...
		for node in nodes_parent.get_nodes():
			if node.is_unlocked() and node.progression_tier > v:
				print("Cannot change current tier as there are unlocked nodes with a progression tier that would be invalid at the new tier.")
				return
		current_tier = v
...
var _changing_by_code: bool = false
